...
parser.add_argument(
    "--EXPO_MAP_MAX_mHz",
    help="the maximum value of the exponential mapping LUT in millihertz, default 20Hz max",
    nargs="?",
    default=20000,
    type=float,
)
...
namespace Lookup_Tables\n{\n"
    ...
    expo_LUT = get_expo_map_lut(
        args.EXPO_MAP_LUT_SIZE,
        args.EXPO_MAP_CURVE,
        args.EXPO_MAP_MIN_mHz,
        args.EXPO_MAP_MAX_mHz,
    )
...
if args.action == "plot-sin":
    ...
    plt.plot(expo_lut / 1000)

    plt.suptitle(
        f"Exponential mapping lookup table with {args.EXPO_MAP_LUT_SIZE} points"
    )
    plt.title(
        f"Min value: {expo_lut.min() / 1000} Hz, Max value: {expo_lut.max() / 1000} Hz"
    )
    plt.xlabel("LUT index")
    plt.ylabel("value (Hz)")
